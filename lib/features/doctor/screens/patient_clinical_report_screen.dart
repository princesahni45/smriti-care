          ...
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Clinical Report formatted for printing/export.')),
              );
            },
          ),
        ],
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Divider(height: 1, thickness: 1.5, color: AppColors.cardBorder),
              const SizedBox(height: 16),
              ...
              const SizedBox(height: 10),
              _buildTwoColTable([
                ('Latest Composite Score', summary.latestScore != null ? '${summary.latestScore} / 100' : 'No data'),
                ('Previous Baseline Score', summary.previousScore != null ? '${summary.previousScore} / 100' : 'No data'),
                ('Recent Trend', summary.trendPercent != null ? '${summary.trendPercent! > 0 ? '+' : ''}${summary.trendPercent!.toStringAsFixed(1)}%' : 'Stable'),
                ('Total Recorded Sessions', '${summary.gameHistory.length} sessions'),
              ...
              if (summary.gameHistory.isEmpty)
                const Text('No cognitive games recorded.', style: TextStyle(fontSize: 12, color: AppColors.muted))
              else
                ...summary.gameHistory.take(5).map((g) => Padding(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('• ${g.gameName} (${g.difficulty})', style: const TextStyle(fontSize: 12, color: AppColors.ink)),
                          Text('Score: ${g.score}/100 • Acc: ${g.accuracy}%', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                        ],
                      ),
                    )),
              const SizedBox(height: 20),
              ...
              if (summary.mriScans.isEmpty)
                const Text('No MRI scans uploaded for this patient.', style: TextStyle(fontSize: 12, color: AppColors.muted))
              else
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text('Prediction: ${m.prediction}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                              Text(m.isReviewed ? 'CLINICALLY REVIEWED' : 'REVIEW PENDING', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: m.isReviewed ? const Color(0xFF2E7D32) : const Color(0xFFE65100))),
                            ],
                          ),
                          const SizedBox(height: 2),
                          Text('Confidence: ${(m.confidenceScore * 100).toStringAsFixed(1)}% • Scan Date: ${_formatDate(m.timestamp)}', style: const TextStyle(fontSize: 11, color: AppColors.muted)),
                        ],
                      ),
                    )),
              const SizedBox(height: 20),
              ...
              if (summary.caregiverObservations.isEmpty)
                const Text('No caregiver observations recorded.', style: TextStyle(fontSize: 12, color: AppColors.muted))
              else
                ...summary.caregiverObservations.map((o) => Padding(
                      padding: const EdgeInsets.only(bottom: 6),
                      child: Text('• ${o.title}: "${o.message}"', style: const TextStyle(fontSize: 12, color: AppColors.ink)),
                    )),
              const SizedBox(height: 20),
              ...
              if (summary.doctorNotes.isEmpty)
                const Text('No doctor notes recorded yet.', style: TextStyle(fontSize: 12, color: AppColors.muted))
              else
                ...summary.doctorNotes.map((n) => Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('${n.doctorName} (${_formatDate(n.createdAt)}):', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.tealDark)),
                          Text(n.text, style: const TextStyle(fontSize: 12, color: AppColors.ink)),
                        ],
                      ),
                    )),
      ...
      child: Column(
        children: rows.map((r) => Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(r.$1, style: const TextStyle(fontSize: 12, color: AppColors.muted)),
