    ...
    if (idx != -1) {
      ...
      try {
        ...
        final syncEvent = SyncEvent(
          id: 'evt_rem_ack_${id}_${now.millisecondsSinceEpoch}',
          eventType: 'reminder_acknowledgement',
          dedupKey: 'rem_ack_${id}_$dateKey',
          payload: {
            'reminderId': id,
            'title': cur.title,
            'type': cur.type,
            'scheduledTime': cur.scheduledTime,
        ...
        debugPrint('CaregiverService queueEvent warning: $e');
      }
      return true;
    }
    return false;
  ...
  Future<void> triggerSosAlert(String triggerType, {String notes = ''}) async {
    ...
    _alerts.insert(
      ...
      CaregiverAlert(
        ...
        message: 'Patient triggered an SOS alert via $triggerType.',
        timestamp: now,
        acknowledged: false,
      ),
    );
  ...
  Future<void> resolveSosAlert(String id) async {
    ...
    if (idx != -1) {
      ...
      _sosLogs[idx] = SosAlertLog(
        ...
        resolved: true,
        notes: '${_sosLogs[idx].notes} (Resolved by caregiver at $now)',
      );
      await _persist();
    }
  }
