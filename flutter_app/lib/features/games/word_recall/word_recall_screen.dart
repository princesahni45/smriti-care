...
class _WordRecallScreenState extends State<WordRecallScreen> {
  ...
  void _setupGame(int levelId) {
    ...
    setState(() {
      ...
      _immediateOptions = List.from(allOptions);
      _delayedOptions = List.from(allOptions)..shuffle(Random());
  ...
  RecallScore _scoreSelection(List<String> selected, List<String> targets) {
    final correct = targets.where((w) => selected.contains(w)).toList();
    final incorrect = selected.where((w) => !targets.contains(w)).toList();
    final missed = targets.where((w) => !selected.contains(w)).toList();
    final accuracy = targets.isNotEmpty
        ? ((correct.length / targets.length) * 100).round()
        : 0;
